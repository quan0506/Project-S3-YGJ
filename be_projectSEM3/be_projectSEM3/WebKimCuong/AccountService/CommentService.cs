    ...
    public class CommentService : ICommentService
    {
        ...
        public async Task<PayLoad<object>> FindAll(int idPR, int page, int pageSize)
        {
            try
            {
                ...
                var checkPr = _context.ProdMsts.Where(x => x.id == idPR).FirstOrDefault();
                if (checkPr == null)
                {
                ...
                //	return await Task.FromResult(PayLoad<object>.NotFound());
                //}
                //            var list = _context.comments.Include(a => a.accounts).ThenInclude(d => d.Reviews).Include(p => p.products).Include(x => x.commentDescriptions).ThenInclude(ac => ac.account).Where(x => x.products == checkPr).Select(x => new
                //            {
                //                ids1 = x.id,
                //                message = x.message,
                //                accountName = x.accounts.username,
                //                danhgia = x.accounts.Reviews.Sum(da => da.sao),
                //                image = x.images,
