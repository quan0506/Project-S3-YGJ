...
namespace thuongmaidientus1.AccountService
{
    public interface IOrderService
    {
        ...
        Task<PayLoad<XulydonhangDTO>> UpdateDonHangXuLy(int product_id, string? name, Status? status = Status.Pending);
        Task<PayLoad<object>> FindAllDonHangXuLy(string? name, int page = 1, int pageSize = 20);
        Task<PayLoad<object>> FindAllDonHangByMax(string? account_name, int page = 1, int pageSize = 20);
        Task<PayLoad<object>> FindAllAccountDonHangByMax(string? account_name, int page = 1, int pageSize = 20);
        ...
        Task<PayLoad<Models.OrderProcessing>> FindOneId(int id);
        Task<PayLoad<object>> AccountByMax();
        Task<PayLoad<object>> TotalMonny();
        Task<PayLoad<object>> FindOnDonHangXuLy(int id);
        Task<PayLoad<object>> FindAllDonHangOfAccount(int id, int page = 1, int pageSize = 20);
    }
