        ...
        public async Task<PayLoad<ShopDTO>> AddShop(ShopDTO shop) 
        {
            try
            {
                ...
                foreach (var item in shop.vanchuyens)
                {
                    ...
                    {
                        var checkVanchuyen = _dbcontext.Shippings.Where(x => x.id == songuyen).FirstOrDefault();
                        if (checkVanchuyen != null)
                        {
                            ...
                            if (int.Parse(intCoverString) > sum - 1)
                            {
                                ...
                                //MapShop.vanchuyen = shop.Shipping[0] // "shop.Shipping[0]": "[0]" nghĩa là lấy ra phần tử đầu tiên trong mảng;
                            }
                    ...
                    if (!checkChuoi)
                    {
                        var checkVanchuyens = _dbcontext.Shippings.Where(x => x.name == item).FirstOrDefault();
        ...
        public async Task<PayLoad<ShopDTO>> EditShop(int id, ShopDTO shop, string? username)
        {
            try
            {
                ...
                for(var i = 0; i < shop.vanchuyens.LongCount(); i++)
                {
                    ...
                    {
                        var checkId = _dbcontext.Shippings.Where(x => x.id == songuyen).FirstOrDefault();
                        if(checkId != null)
                        {
                            sum += 1;
                            if(sum == 1)
                    ...
                    {
                        var checkName = _dbcontext.Shippings.Where(x => x.name == data).FirstOrDefault();
                        if (checkName != null)
                        {
                            listVanChuyenShop.Add(new ShopVanchuyen
                            {
