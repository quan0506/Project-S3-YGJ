        ...
        public async Task<PayLoad<XulydonhangDTO>> AddDonHangXuLy(IList<XulydonhangDTO> xulydonhangs)
        {
            try
            {
                ...
                for (var i = 0; i < xulydonhangs.Count; i++)
                {
                    ...
                    var checkDonhang = _dbcontext.OrderProcessings.Include(a => a.Account).Where(x => x.product_id == data.product_id && x.Account.id == data.account_id && !x.Deleted).FirstOrDefault();
