...
namespace thuongmaidientus1.AccountService
{
    public class VanChuyenService : IVanChuyenService
    {
        ...
        public async Task<PayLoad<Shipping>> AddVanChuyen(Shipping vanchuyen)
        {
            try
            {
                vanchuyen.name = vanchuyen.name.ToLower(); // Chuyển hết sang chữ thường(không viết hoa)
                var checkName = _dbcontext.Shippings.Where(x => x.name.ToLower() == vanchuyen.name && !x.Deleted).FirstOrDefault(); // Chuyển hết dữ liệu về chữ thường để kiểm tra xem đã tồn tại chưa
                if(checkName != null) {
                    return await Task.FromResult(PayLoad<Shipping>.CreatedFail("Tên công ty vận chuyển đã tồn tại"));
                }

                vanchuyen.Deleted = false;
                _dbcontext.Shippings.Add(vanchuyen);
                if(await _dbcontext.SaveChangesAsync() <= 0)
                {
                    return await Task.FromResult(PayLoad<Shipping>.CreatedFail("Faild"));
                }
                return await Task.FromResult(PayLoad<Shipping>.Successfully(vanchuyen));
            }
            catch(Exception ex)
            {
                return await Task.FromResult(PayLoad<Shipping>.CreatedFail(ex.Message));
            }
        }
        ...
        public async Task<PayLoad<string>> DeleteVanChuyen(IList<string> id)
        {
            ...
            for(var i = 0; i < id.Count; i++)
            {
                ...
                {
                    var checkId = _dbcontext.Shippings.FirstOrDefault(x => x.id == chuyenId);
                    if(checkId != null)
                    {
                        checkId.Deleted = true;
                        _dbcontext.Shippings.Update(checkId);
                    }
                    
                }
                if(!checkInt)
                {
                    var checkName = _dbcontext.Shippings.FirstOrDefault(x => x.name == data);
                    if(checkName != null)
                    {
                        checkName.Deleted = true;
                        _dbcontext.Shippings.Update(checkName);
                    }
                }
            }
        ...
        public async Task<PayLoad<Shipping>> EditVanChuyen(int id, Shipping vanchuyen)
        {
            try
            {
                var checkId = _dbcontext.Shippings.Where(x => x.id ==  id).FirstOrDefault();
                if(checkId == null)
                {
                    return await Task.FromResult(PayLoad<Shipping>.CreatedFail("Chưa tồn tại"));
                }

                var checkNameVanChuyen = _dbcontext.Shippings.Where(x => x.name != checkId.name && x.name == vanchuyen.name).FirstOrDefault();
                ...
                return await Task.FromResult(PayLoad<Shipping>.CreatedFail("Update Faild"));
            }
            catch(Exception ex)
            {
                return await Task.FromResult(PayLoad<Shipping>.CreatedFail(ex.Message));
        ...
        public async Task<PayLoad<object>> FindAll(string? name, int page = 1, int pageSize = 20)
        {
            ...
            var data = _dbcontext.Shippings.Where(x => !x.Deleted).AsQueryable();
        ...
        public async Task<PayLoad<object>> FindOneIdOrName(IList<string> id)
        {
            try
            {
                ...
                for(var i = 0; i < id.Count; i++)
                {
                    ...
                    {
                        var checkId = _dbcontext.Shippings.Where(x => x.id == songuyen && !x.Deleted).FirstOrDefault();
                        if(checkId != null)
                        {
                    ...
                    if (!check)
                    {
                        var checkName = _dbcontext.Shippings.Where(x => x.name.Contains(data) && !x.Deleted).ToList();
                        if(checkName != null)
                        {
