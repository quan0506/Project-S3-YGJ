...
namespace thuongmaidientus1.AccountService
{
    public class ProductService : IProductService
    {
        ...
        public async Task<PayLoad<ProductDTO>> AddProduct(ProductDTO productDTO)
        {
            try
            {
                var checkName = _dbcontext.ProdMsts.Where(x => x.title == productDTO.title).FirstOrDefault();
                if (checkName != null)
                {
                ...
                if (await _dbcontext.SaveChangesAsync() > 0)
                {
                    ...
                    {
                        var checkCategoryProduct = _dbcontext.ProductCategorys.Include(p => p.Product).Include(c => c.Category).Where(x => x.Product == productId).ToList();
                        var productCategory = new ProductCategory();
                        if (checkCategoryProduct.Count > 0)
                        {
                            _dbcontext.ProductCategorys.RemoveRange(checkCategoryProduct);
                            if (await _dbcontext.SaveChangesAsync() > 0)
                            {
                                for (var i = 0; i < productDTO.category.Count(); i++)
                                {
                                    ...
                                    {
                                        var CheckIntCategory = _dbcontext.Categorys.Where(x => x.id == songuyen).FirstOrDefault();
                                    ...
                                    if (!checkInt)
                                    {
                                        var checkNameCategory = _dbcontext.Categorys.Where(x => x.name == data).FirstOrDefault();
        ...
        public async Task<PayLoad<ProductDTO>> EditProduct(int id, ProductDTO productDTO, string? name)
        {
            try
            {
                var checkId = _dbcontext.ProdMsts.Where(x => x.id == id).FirstOrDefault();
                if (checkId == null)
                {
                ...
                if (productDTO.category.Any() || productDTO.category.Count > 0)
                {
                    foreach (var item in productDTO.category)
                    {
                        if (int.TryParse(item, out int songuyen))
                        {
                            var checkInt = _dbcontext.Categorys.Where(x => x.id == songuyen).FirstOrDefault();
                            if (checkInt != null)
                            {
                        ...
                        {
                            var checkName = _dbcontext.Categorys.Where(x => x.name == item).FirstOrDefault();
                            if (checkName != null)
                            {
                                if (checkDataList == null || checkDataList.Count <= 0)
                                {
                ...
                if (await _dbcontext.SaveChangesAsync() > 0)
                {
                    ...
                    {
                        var checkCategoryProduct = _dbcontext.ProductCategorys.Include(p => p.Product).Include(c => c.Category).Where(x => x.Product == idProduct).ToList();
        ...
        public async Task<PayLoad<object>> FindAll(string? name, string? account_name, string? type, int page = 1, int pageSize = 10)
        {
            try
            {
                ...
                foreach (var item in data)
                {
                    ...
                    var checkCategory = _dbcontext.ProductCategorys.Include(p => p.Product).Include(c => c.Category).Where(x => x.Product == item).ToList();
                    for (var i = 0; i < checkCategory.Count; i++)
                    {
                        var dataCategory = checkCategory[i];
                        var checkIdCategory = _dbcontext.Categorys.Where(x => x.Equals(dataCategory.Category)).FirstOrDefault();
        ...
        public async Task<PayLoad<object>> FindOne(int id, string? name)
        {
            try
            {
                ...
                var checkCategory = _dbcontext.ProductCategorys.Include(p => p.Product).Include(c => c.Category).Where(x => x.Product == checkId).ToList();
                if (checkCategory != null)
                {
                    ...
                    for (var i = 0; i < checkCategory.Count; i++)
                    {
                        ...
                        var checkCategoryName = _dbcontext.Categorys.Where(x => x.Equals(data.Category)).FirstOrDefault();
                ...
                productDTOs.soluong = checkId.soluong;
        ...
        public async Task<PayLoad<object>> FindAllProductCategory(IList<string> id, int page = 1, int pageSize = 20)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < id.Count; i++)
                {
                    ...
                    var data = id[i];
                    bool checkInt = Regex.IsMatch(data, @"^\d+$");
                    var checkCategory = _dbcontext.Categorys.Where(x => !x.Deleted).ToList();
                    if (int.TryParse(data, out int songuyen))
                    {
                    ...
                    for (var categoryFindOne = 0; categoryFindOne < checkCategory.Count; categoryFindOne++)
                    {
                        ...
                        var checkProductItem = _dbcontext.ProdMsts.Include(c => c.Categorys).Include(s => s.Shops).Where(x => x.Categorys.Equals(dataItemCaytegoru) && x.Deleted == false).ToList();
                        if (checkProductItem.Count > 0)
                        {
                            foreach (var item in checkProductItem)
                            {
                                ...
                                if (!listCheck.Contains(item.title))
                                {
                                    ...
                                    var checkCategoryItem = _dbcontext.ProductCategorys.Include(c => c.Category).Include(p => p.Product).Where(x => x.Product.Equals(item)).ToList();
                                    if (checkCategoryItem.Count > 0)
                                    {
                                        for (var itemCategory = 0; itemCategory < checkCategoryItem.Count; itemCategory++)
                                        {
                                            ...
                                            var categoryItem = _dbcontext.Categorys.Where(x => x.Equals(dataCategory.Category)).FirstOrDefault();
        ...
        public async Task<PayLoad<int>> TotalTonKho()
        {
            try
            {
                var data = _dbcontext.ProdMsts.Sum(x => x.soluong);
